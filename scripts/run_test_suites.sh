...
fast|regtest_cap_rejection_tests|180|FIXED IN PR #129, quarantined only until it merges -- red on main by design, green on #129, both measured with one unchanged test. It asserts that a linear chain longer than nMapBlockIndexCap is accepted in full through ChainSelectorAdapter::ProcessNewHeader. On main (ae1e738b): exit 1, the header at height 1000 REJECTED, index size 1000, cap 1000, no spam and no attacker -- the eviction trigger (chain_selector_impl.cpp:262-268) is purely count-based and the victim search (chain.cpp:220-275) skips every active-chain entry, so a synced regtest node past height 1000 has no victim and ProcessNewHeader fails closed. On fix/119-eviction-uaf-leaf-only: exit 0, 1002 headers accepted, because #129 makes the cap ADVISORY -- when no unpinned leaf remains it logs a rate-limited NOTE and falls through rather than rejecting (chain_selector_impl.cpp:284-306). DO NOT lift this by raising the regtest cap: chainparams.cpp:754-757 says 1000 is deliberate so cap-saturation tests can exercise eviction without flooding 500K headers, and raising it deletes the only place eviction is exercised. Lift when #129 merges.|
