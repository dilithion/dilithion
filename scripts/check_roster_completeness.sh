...
exempt_reason() {
    case "$1" in
        ...
        blockindex_uaf_asan_arm)
            echo "KIND 3, sanitizer-only, and with the sharpest form of that tell: TWO OF ITS THREE ARMS MUST CRASH. Rostering it live would build it WITHOUT -fsanitize=address, where both trap arms read stale bytes and exit 0 -- the roster would print a green for a binary that cannot detect anything. WHERE IT ACTUALLY RUNS, named so this is not 'run elsewhere' with no elsewhere: .github/workflows/ci.yml, job 'AddressSanitizer (Memory Safety)', step 'Deferred-reclamation ASan arms (2 of 3 MUST trap)', on every PR. Driver: scripts/asan_uaf_arms.sh, which refuses to report a verdict without the sanitizer and, with REQUIRE_ASAN=1 as that step sets it, HARD-FAILS instead of skipping. Verdict at 4ae2798c (run 34424068632, job 102705495467): deferred exit 0 clean; immediate exit 1 heap-use-after-free; drained exit 1 heap-use-after-free; 'ASan arms: PASS (0 failed)'. Reproduce: CXX=clang++ CXXFLAGS='-fsanitize=address -fno-omit-frame-pointer -g -std=c++17' LDFLAGS=-fsanitize=address make blockindex_uaf_asan_arm && REQUIRE_ASAN=1 bash scripts/asan_uaf_arms.sh" ;;
        *) echo "" ;;
    esac
}
