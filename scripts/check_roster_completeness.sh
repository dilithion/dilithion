...
exempt_reason() {
    case "$1" in
        ...
        epoch_nest_probe)
            echo "KIND 1, a paired CONTROL whose PASS IS A PROCESS ABORT -- same class as batch_verifier_race_control. It opens an illegal nested EpochOfflineScope, which must fire a ConsensusInvariant; a suite cannot assert that, because catching the invariant is exactly what the invariant prevents, so the refusal is only observable from OUTSIDE the process. Run by scripts/red_arms_pr198_r1_folds.sh as the POSITIVE CONTROL for the inverted F13 mutation arm: the mutant shows the nest is accepted without the refusal, this shows it is rejected with it, and either half alone proves nothing. Rostering a binary whose correct behaviour is to die would file a working control as a defect." ;;
        *) echo "" ;;
    esac
}
