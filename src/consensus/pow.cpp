    ...
    int activationHeight)
{
    ...
    // In standard C++ that overflow is undefined behaviour; $(CONSENSUS_CXXFLAGS) = -fwrapv
    // (see the CONSENSUS-CRITICAL block in the Makefile) is what makes it a defined
    // two's-complement wrap and therefore identical across the GCC/Linux, GCC/MSYS2 and
    // Clang/macOS binaries we ship. Removing -fwrapv re-opens a consensus split here.
    //
