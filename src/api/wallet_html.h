...
<head>
    ...
    <!-- node-wallet-selfcontained: everything between these two markers is
         STRIPPED by the node before it serves this page (StripNodeOnlyBlocks()
         in src/rpc/server.cpp). It is all third-party / PWA chrome that a
         node-served page must not carry:
           - the PWA manifest + service worker describe the HOSTED site's layout
             (start_url=/wallet.html?mode=light, /dilithion-logo-*.png) which the
             node does not serve; worse, sw.js is a network-first cache that
             would persist THIS document — which carries a live ROLE_ADMIN RPC
             session token and is deliberately sent Cache-Control: no-store —
             into CacheStorage, where no-store does not apply.
           - the Google Fonts stylesheet is a third-party request from a
             token-bearing wallet page, and is blocked by the node's CSP
             (style-src 'self') regardless. The node-served page falls back to
             the system font stack; the hosted site keeps its webfonts.
         The hosted copy of this file (website/) keeps all of it verbatim. -->
    <!--DILITHION_NODE_STRIP_BEGIN-->
    <!-- PWA -->
...
//# sourceMappingURL=qrcode.min.js.map</script>
    <!-- node-wallet-selfcontained: SHA3 + ethers are VENDORED (js/vendor/), not
         pulled from a CDN. A wallet page must not fetch crypto/signing code from
         a third party — a compromised CDN would be serving signing code straight
         into users' wallets — and the node's own CSP (script-src 'self') blocks
         it outright, which is what broke the node-served wallet.
         The vendored bytes are the EXACT pinned CDN artifacts: the integrity
         attributes below are the original jsDelivr SRI pins, unchanged, and the
         browser still enforces them against the locally served file, so a
         tampered vendored copy fails closed. Re-verify with:
           openssl dgst -sha384 -binary website/js/vendor/sha3.min.js | openssl base64 -A
         SHA3   = js-sha3@0.9.3/build/sha3.min.js
         ethers = ethers@5.7.2/dist/ethers.umd.min.js -->
    <script src="js/vendor/sha3.min.js"
            integrity="sha384-+JAeTqUiH9WuByObPHouP4rQKYUTLQ9AQW4HHJZ4uNmkA2+nkDpbssQNGsVXnJpe"></script>
    <script src="js/vendor/ethers.umd.min.js"
            integrity="sha384-Htz1SE4Sl5aitpvFgr2j0sfsGUIuSXI6t8hEyrlQ93zflEF3a29bH2AvkUROUw7J"></script>
...
<body>
    ...
    <script>
        ...
        // node-wallet-selfcontained: NEVER on a node-served page. sw.js is a
        // network-first cache that would put THIS document — which carries a
        // live ROLE_ADMIN RPC session token and is served Cache-Control:
        // no-store — into CacheStorage, where no-store has no effect. That
        // would persist an admin credential to disk and hand it back on later
        // loads. The node also does not serve /sw.js at all, so without this
        // guard the registration just 403s into the console.
        if ('serviceWorker' in navigator && !servedOrigin) {
